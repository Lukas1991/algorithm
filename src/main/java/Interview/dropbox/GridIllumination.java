...
//灯可以照到自己行和自己列以及所在两条对角线的所有点。给定一组（m 长度 array）施工位置的二维坐标，该位置及周围8个格的灯要关掉（施工完恢复）。问还有没有灯能照到当前施工位置，返回m长度array。
public class GridIllumination {
    ...
    public List<String> checkIllumination(int n, List<List<Integer>> lamps, List<List<Integer>> queries) {
        ...
        for (List<Integer> lamp : lamps) {
            ...
            if (!leftDiagMap.containsKey(r - c)) {
                leftDiagMap.put(r - c, 1);
            } else {
                leftDiagMap.put(r - c, leftDiagMap.get(r - c) + 1);
            }

            // Update the right diagonal lamp count
            if (!rightDiagMap.containsKey(r + c)) {
                rightDiagMap.put(r + c, 1);
            } else {
                rightDiagMap.put(r + c, rightDiagMap.get(r + c) + 1);
            }
        }
