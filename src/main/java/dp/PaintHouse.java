...
public class PaintHouse {
     ...
     * minimum cost to paint all houses.
     *
     * Could you solve it in O(nk) runtime, in O(1) space?
     *
     ...
     *
     * use costs array as the dp array will reduce the space to O(1)
     */
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < k; j++) {
                if (i == 0) {
                    ...
                    if (j == preMin1Index) {
                        costs[i][j] = costs[i - 1][preMin2Index] + costs[i][j];
                    } else {
                        costs[i][j] = costs[i - 1][preMin1Index] + costs[i][j];
                    }
                }

                //re-calculate min1, min2
                if (currMin1Index < 0 || costs[i][j] <= costs[i][currMin1Index]) {
                    currMin2Index = currMin1Index;
                    currMin1Index = j;
                } else if (currMin2Index < 0 || costs[i][j] < costs[i][currMin2Index]) {
                    currMin2Index = j;
                }
            }
        }

        return costs[n - 1][currMin1Index];
    }
