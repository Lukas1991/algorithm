...
public class BSTree {
    ...
    //return -1 if q or p not in the tree
    private int findDistance(Node root, int p, int q) {
        if (p > q) {
            return findDistance(root, q, p);
        }
        ...
        if (root.data > p && root.data > q) {
            ...
            int leftCount = getPath(root, p);
            int rightCount = getPath(root, q);
    ...
    private int getPath(Node root, int p) {
        Node current = root;  //start at root
        int count = 0;
        while (current != null) { //while not to leaf
            if (current.data == p) {
    ...
    public static void main(String[] args) {
        ...
        System.err.println(bsTree.findDistance(bsTree.root, 2, 4));
        System.err.println(bsTree.findDistance(bsTree.root, 2, 7));
    }
}
