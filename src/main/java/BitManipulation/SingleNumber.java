...
public class SingleNumber {
    ...
    /**
     * Give an array, all the numbers appear twice except one number which appears once and
     * all the numbers which appear twice are next to each other. Find the number which appears once.
     * 二分长度，通过奇偶性找出落单的数
     */
    public int getSingleNumber(int[] nums) {
        ...
        while (left < right) {
            ...
            if (nums[mid] == nums[mid - 1]) {//中点跟前一个相等
                int len = mid - left + 1;
                if (len % 2 == 0) {//中点前面没有single number
                    left = mid + 1;
                } else {
                    right = mid - 2;
                }
